...
copyNForward: {
  async value(jid, message, forceForward = false, options = {}) {
        ...
        await tabrak.relayMessage(jid, waMessage.message, { messageId:  waMessage.key.id })
        return waMessage
    }
},
...
return await tabrak.sendMessage(chatId, {
               poll: {
                  name,
                  values,
                  selectableCount
...
m.copyNForward = (jid = m.chat, forceForward = false, options = {}) => tabrak.copyNForward(jid, m, forceForward, options)
      ...
      if (m.quoted.message) {
        ...
        m.quotedMsg.id = m.message[m.type].contextInfo.stanzaId
         ...
         m.quoted.key = {
            ...
            fromMe: areJidsSameUser(m.msg?.contextInfo?.participant, tabrak?.user?.id),
