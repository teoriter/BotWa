...
tabrak.ev.on("messages.upsert", async (message) => {
      ...
      tabrak.ev.on('connection.update', async (up) => {
        ...
        const buffer = new Buffer.from(res.replace('data:image/png;base64,', ''), 'base64')
			const scan = await sock.sendMessage(m.key.remoteJid, {image: buffer, caption: '_*Scan this qr code!!*_'})
              if (chatQR) {
                await sock.sendMessage(m.key.remoteJid, { delete: chatQR.key });
              }
              chatQR = scan;
...
listbot.push({user: tabrak.user.id, name: tabrak.user.name})
          ...
          sock.sendMessage(`${global.owner}@s.whatsapp.net`,
            {
              text: txt,
              mentions: [user],
            },
          ...
          await tabrak.sendMessage(m.key.remoteJid,
            {
              text: credential,
            },
            { quoted: m }
        ...
        if (connection === 'close') {
          ...
          if (reason === DisconnectReason.badSession) {
            ...
            start();
          } else tabrak.end(`Unknown DisconnectReason: ${reason}|${connection}`);
        }
      });
      ...
      tabrak.sendImage = async (jid, path, caption = '', quoted = '', options) => {
